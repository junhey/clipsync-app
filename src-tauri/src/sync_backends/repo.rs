...
pub struct IndexEntry {
    ...
    /// For image entries: the content-sha of the actual PNG bytes uploaded as
    /// `blobs/<sha[0:2]>/<sha>`. Lets the receiving side fetch the binary
    /// lazily when the user clicks the item.
...
fn content_sha_bytes(body: &[u8]) -> String {
    let mut h = Sha256::new();
    h.update(body);
    hex::encode(h.finalize())
}
...
pub async fn sync_repo_once(
    ...
    //    - to_upload: HashMap<sha, (body, is_binary)>  (only blobs not already remote)
    let mut entries: Vec<IndexEntry> = Vec::with_capacity(items.len());
    let mut to_upload_text: HashMap<String, String> = HashMap::new();
    let mut to_upload_binary: HashMap<String, Vec<u8>> = HashMap::new();
    for it in items.into_iter() {
        if it.kind == "image" {
            // Read the local PNG bytes and content-address them. The receiver
            // can then download `blobs/<sha[0:2]>/<sha>` and reconstruct the
            // image — without this step, image sync would only carry the
            // metadata (width/height/placeholder) and never the actual pixels.
            let png_sha_opt = match crate::storage::read_image_blob(app, &it.id) {
                Ok(bytes) if !bytes.is_empty() => {
                    let png_sha = content_sha_bytes(&bytes);
                    let path = blob_path(&png_sha);
                    if !existing_blobs.contains(&path) {
                        to_upload_binary.entry(png_sha.clone()).or_insert(bytes);
                    }
                    Some(png_sha)
                }
                // No local blob (e.g. an image item pulled from another device
                // that we haven't downloaded yet) — preserve whatever remote
                // ref the item already carried so we don't lose it on push.
                _ => it.remote_ref.clone(),
            };
            entries.push(IndexEntry {
                id: it.id.clone(),
                kind: it.kind,
                // `r#ref` doubles as both the legacy id-ref and (when we have
                // the binary) a pointer to the PNG blob path. We keep `id`
                // here for back-compat readers that haven't been updated yet.
                r#ref: png_sha_opt.clone().unwrap_or_else(|| it.id.clone()),
                text: it.text,
        ...
        if !existing_blobs.contains(&path) {
            to_upload_text.entry(csha.clone()).or_insert(body.clone());
        }
        entries.push(IndexEntry {
            id: it.id.clone(),
            kind: it.kind,
    ...
    fn rewrite_snippets(
        ...
        to_upload_text: &mut HashMap<String, String>,
            ...
            .map(|n| match n {
                crate::snippets::SnippetNode::Folder { id, name, children } => {
                    crate::snippets::SnippetNode::Folder {
                        id,
                        name,
                        children: rewrite_snippets(children, existing_blobs, to_upload_text),
                    }
                }
                crate::snippets::SnippetNode::Snippet {
                    ...
                    if !existing_blobs.contains(&blob_path(&csha)) {
                        to_upload_text.entry(csha.clone()).or_insert(content.clone());
                    }
    ...
    let snippets_serialized = rewrite_snippets(snippets, &existing_blobs, &mut to_upload_text);

    // 3) Upload missing blobs and remember their git sha. Text blobs go up as
    //    utf-8, binary blobs (image PNGs) as base64.
    let mut blob_git_shas: HashMap<String, String> = HashMap::new();
    for (csha, body) in &to_upload_text {
        let git_sha = create_blob(token, repo, body, false).await?;
        blob_git_shas.insert(csha.clone(), git_sha);
    }
    for (csha, bytes) in &to_upload_binary {
        let b64 = B64.encode(bytes);
        let git_sha = create_blob(token, repo, &b64, true).await?;
        blob_git_shas.insert(csha.clone(), git_sha);
    }
    ...
    for e in &entries {
        if e.kind == "image" {
            // Only reference a path if we actually have the binary uploaded;
            // legacy items without `remote_ref` shouldn't dangle in the tree.
            if let Some(sha) = e.remote_ref.as_deref() {
                all_paths.insert(blob_path(sha));
            }
        } else {
...
/// Download a single blob's raw bytes by its content SHA.
pub async fn fetch_blob_bytes(token: &str, repo: &str, sha: &str) -> Result<Vec<u8>, String> {
    let path = blob_path(sha);
    let res = ghc(token)
        .get(format!("{API_BASE}/repos/{repo}/contents/{path}?ref={BRANCH}"))
        .send()
        .await
        .map_err(|e| e.to_string())?;
    if !res.status().is_success() {
        return Err(format!("GET blob {sha}: {}", res.status()));
    }
    let v: serde_json::Value = res.json().await.map_err(|e| e.to_string())?;
    let raw = v["content"].as_str().unwrap_or_default().replace('\n', "");
    B64.decode(raw.as_bytes()).map_err(|e| e.to_string())
}

/// Download a single blob's text content by its content SHA.
async fn fetch_blob_content(token: &str, repo: &str, sha: &str) -> Result<String, String> {
    let bytes = fetch_blob_bytes(token, repo, sha).await?;
    String::from_utf8(bytes).map_err(|e| e.to_string())
}

/// Walk a snippet tree and resolve every `@blob:<sha>` content reference by
/// downloading the corresponding blob.  Unknown / missing blobs are left as-is
/// so a partial failure never corrupts the whole tree.
async fn resolve_snippets_blobs(
    token: &str,
    repo: &str,
    nodes: Vec<crate::snippets::SnippetNode>,
) -> Vec<crate::snippets::SnippetNode> {
    ...
    for node in nodes {
        match node {
            SnippetNode::Folder { id, name, children } => {
                let resolved =
                    Box::pin(resolve_snippets_blobs(token, repo, children)).await;
                result.push(SnippetNode::Folder { id, name, children: resolved });
            }
            SnippetNode::Snippet {
                id,
                name,
                ...
                let resolved = if let Some(sha) = content.strip_prefix("@blob:") {
                    fetch_blob_content(token, repo, sha).await.unwrap_or(content)
                } else {
                    content
                };
                result.push(SnippetNode::Snippet {
                    id,
                    name,
                    content: resolved,
                    enabled,
...
/// Fetch and return the snippet tree from the remote `data` branch,
/// with all `@blob:` content references resolved to their actual text.
/// Returns an empty vec if `snippets.json` doesn't exist yet.
pub async fn fetch_repo_snippets(
    token: &str,
    repo: &str,
) -> Result<Vec<crate::snippets::SnippetNode>, String> {
    let res = ghc(token)
        .get(format!(
            "{API_BASE}/repos/{repo}/contents/snippets.json?ref={BRANCH}"
        ))
        .send()
        .await
        .map_err(|e| e.to_string())?;
    ...
    if !res.status().is_success() {
        return Err(format!("GET snippets.json: {}", res.status()));
    }
    let v: serde_json::Value = res.json().await.map_err(|e| e.to_string())?;
    let raw = v["content"].as_str().unwrap_or_default().replace('\n', "");
    let bytes = B64.decode(raw.as_bytes()).map_err(|e| e.to_string())?;
    let s = String::from_utf8(bytes).map_err(|e| e.to_string())?;
    let nodes: Vec<crate::snippets::SnippetNode> =
        serde_json::from_str(&s).map_err(|e| e.to_string())?;
    // Resolve @blob: refs so callers always receive full-text snippets.
    let resolved = resolve_snippets_blobs(token, repo, nodes).await;
    Ok(resolved)
}
