...
pub fn read_history_from_disk(app: &AppHandle) -> Vec<ClipItem> {
    ...
    let mut items: Vec<ClipItem> = match std::fs::read_to_string(&path) {
        Ok(s) => serde_json::from_str(&s).unwrap_or_default(),
        Err(_) => return Vec::new(),
    };
    // One-time sanitization: earlier sync logic summed `hits` on every merge,
    // which drove the counter exponentially up to `u32::MAX` and triggered a
    // `hits += 1` overflow panic in the clipboard watcher. Clamp absurdly
    // large values back into a sane range so the next capture can't crash.
