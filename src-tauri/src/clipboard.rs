...
pub fn try_capture_text(
    ...
    // Best-effort source-app detection so we can skip blacklisted apps
    // (typically password managers). Both backends spawn an external process
    // (osascript / powershell), which is slow — gate the probe on a non-empty
    // blocklist so the common case (no patterns set) pays nothing.
    let patterns = ignore_sources.lock().clone();
    if !patterns.is_empty() {
        #[cfg(target_os = "macos")]
        let source = source_hint_macos();
        #[cfg(target_os = "windows")]
        let source = source_hint_windows();
...
#[cfg(target_os = "windows")]
fn source_hint_windows() -> Option<String> {
    // Cheap, dependency-free probe: shell out to PowerShell to read the
    // foreground window's owning process name. Slower than calling
    // `GetForegroundWindow` + `GetWindowThreadProcessId` directly via
    // `windows-rs`, but doesn't add a heavy dependency for the small
    // population of Windows users.
    //
    // Returns something like "1Password.exe" or "KeePassXC.exe" that the
    // pattern matcher in `source_blocked` can use against the default
    // platform blocklist.
...
"@;
$pid = 0; [void][W]::GetWindowThreadProcessId([W]::GetForegroundWindow(), [ref]$pid);
