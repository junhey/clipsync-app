    ...
    TrayIconBuilder::with_id("main-tray")
        ...
        .on_menu_event(|app, event| {
            ...
            match id {
                ...
                "quit" => app.exit(0),
                "about" => {
                    // Open the main popup and navigate to settings page so the
                    // version info (rendered at the bottom of Settings) is visible.
                    // Avoids JS alert() which fights with the focus-lost-hide logic.
                    toggle_popup(app, None);
                    if let Some(w) = app.get_webview_window("main") {
                        let _ = w.eval("window.__openSettings && window.__openSettings()");
                    }
                }
